...
% f_c' - frequência da portadora detectada
\glsxtrnewsymbol[description={Frequência da portadora detectada}]
{fcprime}
{\ensuremath{f_c'}}

% st' - 
\glsxtrnewsymbol[description={Segmento de sinal modulado em banda passante, em função de tempo $t$}]
{stprime}
{\ensuremath{s'(t)}}

% f_n nominal frequency
\glsxtrnewsymbol[description={$f_c$ randômica selecionada pela PCD, $401.620 \leq f_n \leq 401.650\,\text{MHz}$ (ARGOS-3)}]
{fn}
{\ensuremath{f_n}}
...
% m - índice de amostra
\glsxtrnewsymbol[description={Índice de segmento de sinal}]
{m}
{\ensuremath{m}}

% Nc - número de vezes que a portadora precisa ser detectada para iniciar a recepção
\glsxtrnewsymbol[description={Número de detecções minima para ser considerada como "confirmada", $N_c = 4$}]
{Nc}
{\ensuremath{N_c}}
...
\glsxtrnewsymbol[description={Variância do ruído adicionado ao sinal modulado}]
{sigma2}
{\ensuremath{\sigma^2}}

% Pd - Prefix duration
\glsxtrnewsymbol[description={Duração do prefixo de símbolos para portadora pura, $P_d = f_s * \approx 0.082$ (ARGOS-3)}]
{Pd}
{\ensuremath{P_d}}
...
{mhz}
{\ensuremath{\mathrm{MHz}}}

% kHz kilohertz
\glsxtrnewsymbol[description={\textit{Kilohertz}}]
{khz}
{\ensuremath{\mathrm{kHz}}}

% Hz hertz
\glsxtrnewsymbol[description={\textit{Hertz}}]
{hz}
{\ensuremath{\mathrm{Hz}}}
...
% Sinc function
\glsxtrnewsymbol[description={Função sinc normalizada}]
{sinc}
{\ensuremath{\sinc(x)}}

% Matriz waterfall
\glsxtrnewsymbol[description={Matriz de componentes detectadas, $x[k,m]$}]
{waterfall}
{\ensuremath{x[k,m]}}
...
% cauda do datagrama
\glsxtrnewsymbol[description={Cauda do datagrama, entre sete e nove bits $'0'$ (ARGOS-3)}]
{Em}
{\ensuremath{E_m}}
...
% kmax - índice de amostra de maior correlação com o preâmbulo
\glsxtrnewsymbol[description={Índice de amostra de maior correlação com o preâmbulo}]
{kmax}
{\ensuremath{k_{max}}}

% L_sync - tamanho do preâmbulo em amostras 
\glsxtrnewsymbol[description={Tamanho do preâmbulo em bits, $L_{sync} = 15$ (ARGOS-3)}]
{Lsync}
{\ensuremath{L_{sync}}}
...
% H(f) - resposta em frequência do filtro passa baixa
\glsxtrnewsymbol[description={Resposta em frequência do filtro passa baixa}]
{Hf}
{\ensuremath{H(f)}}

% Hs - função de transferência do filtro passa baixa
\glsxtrnewsymbol[description={Função de transferência do filtro passa baixa}]
{Hs}
{\ensuremath{H(s)}}

% Hz - função de transferência discreta do filtro passa baixa
\glsxtrnewsymbol[description={Função de transferência discreta do filtro passa baixa}]
{Hz}
{\ensuremath{H(z)}}

% db - decibel
\glsxtrnewsymbol[description={\textit{Decibel}}]
{db}
{\ensuremath{\mathrm{dB}}}
...
% u_t', vetor de saída do decodificador convolucional
\glsxtrnewsymbol[description={Vetor de saída do decodificador convolucional}]
{utprime}
{\ensuremath{u[n] \prime}}
