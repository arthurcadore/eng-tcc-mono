...
Apesar da ampla cobertura, a comunicação com satélites de órbita baixa enfrenta o desafio técnico da necessidade de visada simultânea entre a \gls{PCD} e o satélite, condição que reduz a janela de transmissão e restringe a coleta contínua de dados. Além disso, o movimento relativo entre a \gls{PCD} e o satélite provoca o chamado efeito Doppler, responsável por deslocamentos na frequência do sinal recebido, podendo atingir até ±$79{,}4$ \gls{khz}. Esse desvio precisa ser compensado para garantir a correta demodulação do sinal \cite{rae2005detector, rodrigues_demodulador_2018}.
...
A arquitetura dos satélites que compõem a constelação é baseada na integração do transceptor \gls{AD9361} com uma \gls{FPGA} de arquitetura ARM (Zynq-7000), formando uma plataforma de \gls{SDR} altamente flexível \footnote{https://www.argos-system.org/wp-content/uploads/2023/01/ARTIC-Chipset-AnSem-Info-sheet.pdf}. Essa configuração permite a reconfiguração remota do hardware, o que é especialmente importante para futuras atualizações de protocolo ou migração para novos padrões de comunicação, como o \gls{PTT-A3}.
...
A sequência de transmissão do quadro inicia-se com a portadora contínua ou pura, com duração entre 80 e 82 ms. Durante essa etapa a portadora não transmite dados modulados e é utilizada pelo receptor apenas para realizar a detecção do sinal, bem como para facilitar o processo de sincronização de frequência e fase da portadora. 
...
\begin{equation}
\begin{aligned}
    S_I[n] &= [S_0,\ S_2,\ S_4,\ \dots,\ S_{28}] &\mapsto&  S_I[n] = [111111111111111]     \\
    S_Q[n] &= [S_1,\ S_3,\ S_5,\ \dots,\ S_{29}] &\mapsto&  S_Q[n] = [001101010100111]
\end{aligned}
...
Entre os sensores comumente utilizados, destacam-se os de direção do vento ($^{\circ}$), precipitação (mm), pressão atmosférica (mB), radiação solar acumulada (MJ/\ensuremath{\mathrm{m^2}}), temperatura do ar ($^{\circ}\text{C}$), umidade relativa ($\%$), e velocidade do vento (m/s). Por exemplo, os dados \footnote{http://sinda.crn.inpe.br/PCD/SITE/novo/site/historico/action.php} coletados da PCD 31855 são mostrados no \autoref{quadro:dados-meteorologicos}.
...
O campo de tamanho de mensagem \gls{Tm} é utilizado para informar ao receptor quantos blocos de dados estão sendo transmitidos no datagrama. Este campo é formado pelo número de blocos em formato binário \gls{Bm} e pelo bit de paridade \gls{Pm} para fechar uma seção de 4 bits. Como cada bloco possui 32 bits (exceto o primeiro), é possível determinar o comprimento total dos dados de aplicação de \gls{Bm} = ($N_b - 1)_{2}$. O número de blocos \gls{Nb}, pode variar de um a oito, resultando no comprimento esperado de bits no receptor para cada caso, conforme o \autoref{quadro:comprimento-mensagem}. 
...
\begin{quadro}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\centering\arraybackslash}X 
                                  ...
                                  >{\centering\arraybackslash}X}
        \toprule
        \textbf{$N_b$} & \textbf{N° de Bits} & \textbf{$B_m$} & \textbf{$P_m$} & \textbf{$T_m$}\\
        \midrule
        1 & 24  & 000 & 0 & 0000\\
        2 & 56  & 001 & 1 & 0011\\
        3 & 88  & 010 & 1 & 0100\\
...
Conforme apresentado acima, para cada valor de \gls{Bm}, um valor de \gls{Pm} é calculado. O bit de paridade é calculado de forma a garantir que o número total de bits \texttt{1} na mensagem seja par. Ao final, o campo de tamanho de mensagem \gls{Tm} é formado pela concatenação do valor de \gls{Bm} e \gls{Pm}, resultando em um campo de 4 bits \textcite{cnes_services_and_message_formats_ed2_rev2_2006}. 
...
\vspace{-1em}
\begin{equation}
R_{PCD} = \left( \sum_{i=0}^{19} I_{PCD}[i] \cdot 2^i \right) \bmod 256 \text{ ,}
\end{equation}

\noindent   É importante destacar que a proteção contra erros do \gls{pcdid} é dada tanto de forma direta pelo checksum aplicado quanto de forma indireta pelo uso da codificação convolucional aplicada à mensagem como um todo. Além da redundância oferecida pelo número de repetições da mensagem ao longo da passagem do satélite. 


\subsection{Bits de fim de mensagem}

Ao final do datagrama, são inseridos entre sete e nove bits \texttt{0} com a finalidade de limpar o registrador do codificador convolucional, dando o encerramento da sequência codificada. Conforme o padrão \gls{ARGOS-III}, a quantidade de bits de fim de mensagem adicionados depende do comprimento total da mensagem do usuário, conforme apresentado na \autoref{quadro:bits-codificacao}. Apesar de não carregar dados úteis a nível de aplicação, esses bits são fundamentais para o correto funcionamento do processo de decodificação \cite{cnes_services_and_message_formats_ed2_rev2_2006}.

\begin{quadro}[H]
    \caption{Comprimento da cauda \texttt{0} para cada tamanho de mensagem}
    ...
    \begin{tabularx}{\textwidth}{>{\centering\arraybackslash}X 
                                  ...
                                  >{\centering\arraybackslash}X}
        \toprule
        \textbf{$N_b$} & \textbf{Bits Aplicação} & \textbf{Bits Datagrama} & \textbf{N° bits \texttt{0}} \\
        \midrule
        1 &  24 &  56 & 7 \\
        2 &  56 &  88 & 8 \\
        3 &  88 & 120 & 9 \\
...
O transmissor do tipo \gls{PTT-A3} é responsável por processar os dados do datagrama e convertê-los em um sinal modulado \gls{QPSK} para transmissão via satélite. Esse processo envolve várias etapas, incluindo a codificação convolucional, o embaralhamento dos dados, a modulação \gls{QPSK} e a geração do sinal final para transmissão. A seguir, são detalhados os componentes utilizados na estrutura do transmissor \gls{PTT-A3}.
...
No padrão \gls{ARGOS-III}, o código convolucional utilizado segue o padrão \gls{CCSDS} 131.1-G-2. Portanto a taxa de codificação utilizada é $R = \text{1/2}$, o que significa que para cada bit de dados de entrada \gls{ut}, são gerados dois bits de saída, um no canal \gls{cI} e outro no canal \gls{cQ}, aumentando a redundância e melhorando a capacidade do sistema de detectar e corrigir erros. Para a codificação convolucional, são utilizados vetores geradores \gls{G0} e \gls{G1}, de acordo com o padrão CCSDS 131.1-G-2 \cite{cnes_services_and_message_formats_ed2_rev2_2006}. A representação binária dos vetores geradores é dada por
...
Os vetores geradores são utilizados para definir a estrutura do registrador do codificador convolucional aplicado à sequência de entrada \gls{ut}, resultando nas saídas \gls{vt0} e \gls{vt1}, que são utilizadas posteriormente no processo de embaralhamento. Essa operação pode ser representada por uma multiplicação vetorial entre uma janela deslizante de sete bits da entrada e a matriz formada pelos vetores geradores, conforme
...
\begin{aligned}
    ...
    &=
    \begin{bmatrix}
        u[n] & u[n-1] & u[n-2] & u[n-3] & u[n-4] & u[n-5] & u[n-6]
    \end{bmatrix} \cdot \\
    &\quad
    \begin{bmatrix}
        1 & 1 & 1 & 1 & 0 & 0 & 1 \\
...
Após a codificação convolucional, os vetores de saída \gls{vt0} e \gls{vt1} são embaralhados para evitar padrões repetitivos, formando os vetores \gls{Xn} e \gls{Yn} de bits embaralhados. O embaralhador é utilizado para garantir que os dados sejam melhor distribuídos diminuindo a correlação entre os bits de forma a aumentar a aleatoriedade antes da modulação \gls{QPSK}, o que permite atingir uma melhor característica espectral do sinal transmitido \cite{cnes_services_and_message_formats_ed2_rev2_2006}.

O embaralhador é implementado utilizando um registrador de deslocamento de três bits para cada canal, conforme apresentado na \autoref{fig:embaralhador}. A cada novo bit de entrada, o registrador é deslocado, e o bit de saida calculado através de uma multiplexação entre as três saidas possíveis. Desta forma o índice de embaralhamento de uma sequência de entrada é dado por
...
\noindent Esse processo pode ser representado pelo diagrama de blocos apresentado na \autoref{fig:embaralhador} \cite{cnes_services_and_message_formats_ed2_rev2_2006, rodrigues_demodulador_2018}.
...
Em seguida, os vetores de bits embaralhados \gls{Xn} e \gls{Yn}, já multiplexados com os vetores de preâmbulo \gls{SIn} e \gls{SQn}, precisam ser convertidos em vetores de símbolos, para isso é aplicada uma codificação de linha. O vetor \gls{Xn} é codificado utilizando a técnica \gls{NRZ}, enquanto o vetor \gls{Yn} é codificado utilizando a técnica \gls{Manchester} \cite{cnes_services_and_message_formats_ed2_rev2_2006}.

A codificação \gls{NRZ} é realizada removendo a componente DC da sequência de entrada, ou seja, no vetor \gls{Xn} cada bit \texttt{1} é representado por $+1$ e cada bit \texttt{0} é representado por $-1$. Esse processo pode ser descrito pela expressão
...
Para o vetor \gls{Yn}, é aplicada a codificação \gls{Manchester}, onde cada bit de entrada é representado por dois símbolos de saída, alternando entre '-1' e '+1'.  A codificação \gls{Manchester} é utilizada para garantir que haja transições de nível no sinal mesmo em sequências longas de bits iguais, essas transições refletem em mais trocas de símbolos na modulação \gls{QPSK}. Esse processo pode ser descrito pela expressão
...
Uma vez com os vetores de símbolo \gls{In} e \gls{Qn}, é aplicada a modulação de pulso, filtrados para formar uma sequência contínua de símbolos ao longo de \gls{t}, onde cada bit de informação é transmitido durante um período de tempo $\text{ \gls{Tb}} = 1/ \text{\gls{Rb}}$ (tempo de bit), definido com base na taxa de bit \gls{Rb} \cite{cnes_services_and_message_formats_ed2_rev2_2006}.


Para aplicar a modulação de pulso e gerar os sinais analógicos \gls{dI} e \gls{dQ}, os vetores de símbolo são multiplicados com um pulso com resposta ao impulso \gls{gt}. A formatação é dada por 
...
\noindent onde \gls{alpha} é o fator de roll-off do pulso, que controla a largura de banda \gls{W} do sinal modulado, e \gls{sinc} é a função sinc, definida como

\begin{equation}
    \sinc(x) = 
    \begin{cases}
        1, & \text{se } x = 0 \\
        \dfrac{\sin(\pi x)}{\pi x}, & \text{se } x \neq 0
    \end{cases} \text{ .}
\end{equation}
...
Quanto maior o \gls{alpha}, mais suave é a transição entre os símbolos, mas também maior é a largura de banda ocupada. A \autoref{fig:formatter_comp} apresenta a comparação entre os sinais \gls{dI} e \gls{dQ} quando é aplicada a modulação de pulso com pulso \gls{RRC} e \gls{Rect} \cite{10555531840}.

\begin{figure}[H]
	\caption{Comparação entre modulação de pulso RRC e Rect}\label{fig:formatter_comp}
...
\noindent onde, \gls{Tb} é o período de bit. Já com o pulso \gls{RRC}, o sinal é suavizado no dominio do tempo, reduzindo a interferência entre símbolos e ajustando a largura de banda do sinal conforme os parâmetros do modulador de pulso \cite{cnes_services_and_message_formats_ed2_rev2_2006,10555531840,rodrigues_demodulador_2018}.

Dessa forma, podemos calcular a banda consumida (unilateral) do sinal modulado \gls{W} para modulação de pulso \gls{RRC} como 
...
\noindent onde \gls{alpha} é o fator de roll-off do pulso e \gls{Tb} é o período de bit. Já para modulação de pulso \gls{Rect}, a banda de primeiro lóbulo do sinal modulado é dada por
...
\noindent Assim, a largura de banda \gls{W} do sinal modulado em \gls{RRC} considerando um tempo de bit \gls{Tb} = $1 / 400$ e um fator de roll-off $\alpha = 0{,}8$ é de 360 \gls{hz} (banda unilateral), ou seja, $2W = 720 \text{\gls{hz}}$ de banda bilateral, em contraste com os 800 \gls{hz} de banda (bilateral) do sinal modulado em \gls{Rect}. 

% Na \autoref{fig:formatter_freq_comp} é possível observar a diferença na banda consumida pelos sinais \gls{dI} e \gls{dQ} após a modulação de pulso sobre os canais.
...
\vspace{-0.8em}
\begin{equation}
    s(t) = A \cdot d_I(t) \cdot \cos(2\pi f_c t + \phi_0) - A \cdot d_Q(t) \cdot \sin(2\pi f_c t + \phi_0) \text{ ,}
\end{equation}
...
O receptor do tipo \gls{PTT-A3} é responsável por processar o sinal modulado \gls{st} recebido via satélite, extraindo os dados do datagrama transmitido. Esse processo envolve várias etapas, incluindo a detecção da portadora, a demodulação em banda base, a amostragem e decisão dos símbolos, o decodificador convolucional e o desembaralhador. A seguir, são detalhados os componentes utilizados na estrutura do receptor \gls{PTT-A3}. 

\subsection{Detecção de portadora}

Para realizar a detecção da portadora, o sinal recebido acrescido de ruído deve ser inicialmente amostrado e dividido em segmentos discretos \gls{xn} no tempo. O satélite \gls{ARGOS} realiza a decisão do sinal a cada $10$ ms, esse processo é definido por 
...
\noindent onde \gls{xn} representa o segmento de sinal, \gls{st} é o sinal recebido, \gls{m} é o índice de amostra do segmento de tempo e \gls{Tn} é o tempo de amostragem. Em seguida, aplica-se a \gls{FFT} no vetor \gls{xn}, obtendo-se o espectro de frequência do sinal amostrado \gls{xnk}, conforme a expressão 
...
A partir das amostras de \gls{xnk}, se calcula a potência em cada componente de frequência, para obter os valores \gls{Pnk}, conforme
...
Em seguida, para cada índice \gls{k} do espectro calculado, é feita uma comparação com um limiar pré-definido \gls{Pt}. Caso a potência \gls{Pnk} seja maior que \gls{Pt}, a frequência é registrada pois existe a possibilidade de que uma portadora \gls{fn} esteja presente naquela frequência. 

Por fim, no proximo segmento, $X_{n+1}[k]$, o sistema verifica se a frequência registrada no segmento anterior persiste, ou seja, se a potência de $k$ também é maior que o limiar \gls{Pt}, caso positivo, o sistema considera que a frequência \gls{fn} é a portadora \gls{fc} do sinal recebido e repassa para a cadeia de recepção, utilizando \gls{fc} como parâmetro.



\subsection{Demodulador banda base}

Para realizar a demodulação do sinal \gls{stprime} recebido retornando as componentes \gls{dXprime} e \gls{dYprime} em banda base, o sinal \gls{st} é multiplicado por duas portadoras ortogonais, \gls{xi} para demodular o canal \gls{cI} e \gls{yq} para demodular o canal \gls{cQ}. Assumindo sincronismo perfeito, o processo de demodulação para o canal \gls{cI} pode ser expresso como

\vspace{-1.2em}
\begin{equation}
d_X'(t) = \left[A \cdot d_I'(t) \cos(2\pi f_c t ) - A \cdot d_Q'(t) \sin(2\pi f_c t )\right] \cdot 2\cos(2\pi f_c t )
\end{equation}
\vspace{-0.8em}
\begin{equation}
    d_X'(t) =
...
\vspace{-1.2em}
\begin{equation}
d_Y'(t) = \left[A \cdot d_I'(t) \cos(2\pi f_c t ) - A \cdot d_Q'(t) \sin(2\pi f_c t )\right] \cdot 2\sin(2\pi f_c t )
\end{equation}
\vspace{-0.8em}
\begin{equation}
    d_Y'(t) =  
...
\subsection{Filtragem}\label{sec:filtering}

Para isolar os sinais em banda base \gls{dIprime} e \gls{dQprime}, é necessário aplicar um \gls{LPF}, com frequência de corte de $720$ \gls{hz}, com resposta ao impulso \gls{ht}, que remove as componentes de alta frequência em $2 \cdot \text{\gls{fc}}$ resultantes da multiplicação de demodulação realizada anteriormente \cite{rodrigues_demodulador_2018}.

Para o \gls{LPF}, o filtro Butterworth é escolhido por sua resposta em frequência plana e sem ondulações (ganhos ou atenuações) na banda passante, o que é ideal para preservar a integridade do sinal que está passando pelo filtro, neste caso como é um filtro passa baixa, o sinal que está em banda base. A aplicação do filtro \gls{LPF} aos sinais \gls{dXprime} e \gls{dYprime} pode ser expressa como
...
Após o processo de filtragem passa-baixa com o \gls{LPF}, os sinais \gls{dIprime} e \gls{dQprime} em banda base passam por um filtro casado ou \gls{MF}. O \gls{MF} é utilizado para maximizar a relação sinal-ruído (\gls{SNR}) de cada canal, melhorando a detecção dos símbolos transmitidos \cite{10555531840}. O sinal antes da filtragem casada pode se definido como uma soma do sinal transmitido com ruído adicionado, conforme

\vspace{-1em}
\begin{equation}
    d_I(t) = s_I(t) + n(t) \text{ , } d_Q(t) = s_Q(t) + n(t) \text{ ,}
\end{equation}

\noindent onde $n(t)$ é o ruído \gls{AWGN} adicionado ao sinal transmitido. Como queremos maximizar a \gls{SNR} no instante de decisão, o pulso do filtro casado \gls{gt} deve ser definido como inverso no tempo do pulso utilizado na modulação de pulso (conforme apresentado na seção \ref{sec:pulse_modulation}), ou seja 
...
\noindent onde \gls{gt} é o pulso utilizado na modulação de pulso de cada canal. Essa escolha se deve ao fato de que a convolução do sinal recebido com o pulso inverso no tempo do pulso de transmissão resulta em uma maximização da \gls{SNR} no instante de decisão, conforme demonstrado na teoria de detecção de sinais em presença de ruído \cite{10555531840}. A \gls{SNR} do sinal pode ser expressa como

\vspace{-1em}
\begin{equation}
    \bigg(\frac{S}{N}\bigg)_{\!T} =
    \frac{\bigg|\displaystyle\int_{-\infty}^{\infty} S(f)\, G(f)\, e^{-j2\pi f t}\, df\bigg|^2}
    {\dfrac{N_0}{2} \cdot \displaystyle\int_{-\infty}^{\infty} |G(f)|^2\, df} \text{ ,}
\end{equation}


\noindent onde $S(f)$ é a transformada de Fourier do sinal transmitido, $G(f)$ é a transformada de Fourier do pulso do filtro casado, $N_0$ é a densidade espectral de potência do ruído. Para encontrar  a máxima \gls{SNR}, deve-se encontrar o pulso $G(f)$ que maximize a expressão acima, o que é alcançado quando $G(f)$ é proporcional ao conjugado complexo de $S(f)$, ou seja, quando o pulso do filtro casado é o inverso no tempo do pulso de transmissão \cite{10555531840}, conforme 

\vspace{-1em}
\begin{equation}
    G(f) = k \cdot S^*(f) e^{-j2\pi f t} \mapsto g(t) = k \cdot s^*(-t) \text{ ,}
\end{equation}

\noindent onde $k$ é uma constante de proporcionalidade. Assim, a aplicação do filtro casado aos sinais \gls{dXprime} e \gls{dYprime} pode ser expressa como

\vspace{-1em}
\begin{equation}
    \begin{array}{c@{\quad\text{e}\quad}c}
        d_I''(t) = d_I'(t) * g_{\text{MF}}(t) &
        d_Q''(t) = d_Q'(t) * g_{\text{MF}}(t)
    \end{array} \text{ ,}
\end{equation}

Esse processo resulta em uma saída que acumula corretamente a energia do símbolo transmitido, pois está correlacionado com o pulso original \gls{gt}, enquanto que o ruído, que é aleatório e não correlacionado com o pulso, não se acumula da mesma forma, resultando em uma atenuação do ruído em comparação com o sinal. Dessa forma, aumentando a \gls{SNR} e melhorando a detecção dos símbolos transmitidos \cite{10555531840}.

\subsection{Decisão de símbolos}
...
\noindent onde \gls{n} é o índice de amostra, \gls{Tb} é o tempo de bit $1/R_b$ e \gls{tau} é o atraso inicial da decisão. 
% A \autoref{fig:sampling} apresenta os sinais amostrados \gls{Inprime} e \gls{Qnprime}. 
...
\begin{equation} 
    \text{quant}(I'[n]) = \begin{cases} 
    +1, & \text{se } I'[n] \geq 0 \\
    -1, & \text{se } I'[n] < 0
    \end{cases}
    \quad \text{e} \quad
    \text{quant}(Q'[n]) = \begin{cases} 
    +1, & \text{se } Q'[n] \geq 0 \\
    -1, & \text{se } Q'[n] < 0
    \end{cases} \text{ ,}
\end{equation}

\noindent onde \gls{Inprime} e \gls{Qnprime} são os vetores símbolo. 

% A \autoref{fig:comparacao_bits} apresenta os símbolos decididos nos vetores \gls{Inprime} e \gls{Qnprime} após o processo de quantização, em comparação com os vetores originais \gls{In} e \gls{Qn} transmitidos.
...
\noindent resultando no vetor de bits \gls{Xnprime} correspondente ao canal \gls{cI}. O mesmo processo é aplicado ao canal \gls{cQ}, onde a decodificação \gls{Manchester} mapeia os pares de valores $+1,-1$ para o bit \texttt{1} e $-1,+1$ para o bit \texttt{0}, conforme
...
\subsection{Decodificador convolucional}\label{sec:convolutional_decoder}

Após o desembaralhamento, os vetores \gls{vt0prime} e \gls{vt1prime} estão prontos para a decodificação convolucional para obter novamente a sequência \gls{utprime} correspondente ao vetor de bits do datagrama. O algoritmo mais popularmente utilizado para essa etapa é o algoritmo de Viterbi, que implementa a decodificação \gls{MLD} para códigos convolucionais, calculando o caminho mais provável através de \textit{Hamming Distance} para a mensagem recebida em relação à mensagem possível \cite{cnes_services_and_message_formats_ed2_rev2_2006, rodrigues_demodulador_2018}.

Para realizar a decodificação convolucional, o algoritmo de Viterbi utiliza a representação do código na forma de uma treliça (\textit{trellis}), onde cada estado corresponde a uma possível configuração do registrador de deslocamento do codificador. No caso do padrão CCSDS 131.1-G-2, cujo registrador possui memória $K = 7$, o número total de estados é $2^{K-1} = 64$. Em cada instante de símbolo, apenas duas transições possíveis levam a cada estado, representando a entrada $u[n] = 0$ ou $u[n] = 1$.

Para cada transição válida da treliça, o algoritmo calcula uma \textit{métrica de ramo}, que quantifica o grau de semelhança entre o par de bits recebido e o par esperado para aquela transição. Na decodificação \textit{Hard Viterbi}, empregada neste trabalho, essa métrica é dada pela Distância de Hamming entre os bits recebidos e os bits codificados esperados. A Distância de Hamming entre dois vetores binários $a$ e $b$ de comprimento $N$ é definida por

\begin{equation}
    d_H(a,b) = \sum_{i=1}^{N} \left| a_i - b_i \right|,
\end{equation}

onde $d_H(a,b)$ representa o número de posições em que os símbolos diferem. Essa métrica é acumulada ao longo dos estados da treliça, formando a chamada \textit{métrica de caminho}. Em cada novo passo da treliça, o algoritmo mantém apenas o caminho de menor métrica associado a cada estado, descartando caminhos menos prováveis.

Ao final da travessia da treliça, é realizado o procedimento de \textit{traceback}, no qual o algoritmo percorre retroativamente os estados para recuperar a sequência mais provável de bits que foram transmitidos $\gls{utprime}$. A eficiência do algoritmo de Viterbi se deve justamente à eliminação de caminhos improváveis e ao uso da métrica cumulativa, é isso que permite a correção de erros dando ganho à comunicação.

O desempenho do código pode ser medido dirtamente pela distância livre do código $\gls{dfree}$, desde que a quantidade de erros nas transições não passem da capacidade de correção do código, será possivel recuperar oque foi transmitido. Assim, o código convolucional do padrão CCSDS, com $\gls{dfree} = 10$, garante um ganho de desempenho significativo quando comparado a transmissão sem codificaçao, como é possível observar na \autoref{fig:curva_bersnr}.
